import { StandardPlugin, StandardValueGeneratorBuilder } from '@falbricate/base';
...
const regions = [
  // European
  'eu', 'non-eu', 'europe', 'n-europe', 's-europe', 'e-europe', 'w-europe'
]

asPossibilities.forEach((as) => {
  regions.forEach((region) => {
    add(`country-${as}-${region}`, () => new RandomCountryGenerator({
      as, countries: region
    }));
  });
});
